...

{
    ...
    for(i=0; i<count; i++)
    {
        if(strcasecmp(x[i].last,lastname)==0)
        {
            ...
            printf("\n%s,",x[i].first);
            printf("%s,",x[i].last);
            printf("%d-%d-%d,",x[i].dob.day,x[i].dob.month,x[i].dob.year);
 ...
 while(1)
    {
       ...
       scanf("%s",&x);
        ...
        case '7':
            ...
            break;
        case '8':
            printf("All data will be discarded (if not saved),if you still want to continue press 1\n");
            scanf("%s",&y);
            if(y=='1')
                Quit();
            break;
